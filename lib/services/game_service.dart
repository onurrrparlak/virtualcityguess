  ...
  class GameService with ChangeNotifier {
    ...
    int _currentRound = 1;
    bool _gameShouldEnd = false;
    String? _currentTarget;
    String? _videoUrl;
    bool get gameShouldEnd => _gameShouldEnd;
    ...
    Future<void> updateRoundEndedInFirestore(String roomId) async {
      try {
        // Update Firestore document field 'roundEnded' to true
        await FirebaseFirestore.instance.collection('rooms').doc(roomId).update({
          'roundEnded': true,
        });
      } catch (e) {
        print('Error updating roundEnded: $e');
        throw e;
      }
    }
    ...
    Future<void> nextRound(String roomId) async {
      try {
        ...
        await roomRef.update({
          ...
          'roundEnded': false, // Set roundEnded to false
        });

        if (currentRound + 1 == 3) {
          // If the next round is 2, set gameStarted to false
          await roomRef.update({
            'gameStarted': false,
          });
        }
